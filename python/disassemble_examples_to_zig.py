...
const enso = @import("enso");
const OpCode = enso.bytecode.OpCode;
const StaticStringMap = @import("std").StaticStringMap;
const object = enso.object;
...
def argval_to_PyArgVal(val):
    if val is None:
        ...
        return (
            "object.Object{ .string = .{ .string = &[_]u8{"
            + as_hex_string(val)
            + "} } }"
        )
...
def named_codeobject(example_path, co):
    ...
    print(f"pub const {example_name}: object.Code = ", file=output_file)
    print(codeobject_to_zig(co), file=output_file)
...
for example in examples:
    ...
    .{{ "{example.name}", .{{
    	...
    	.captured_stderr = "",
    	.source = @embedFile("{Path(example.path).name}"),
    	.co = &{example.name}, }}
    }},
    """,
        file=output_file,
