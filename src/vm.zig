    ...
    pub fn eval(self: *Self, co: bytecode.CodeObject) !void {
        ...
        for (co.getInstructions()) |insn| {
            switch (insn) {
                ...
                .load_name => |sym| {
                    // Symbol IS the intern index — push it as an object so call() can resolve it.
                    // TODO: proper global/builtin lookup
                    self.push(.{ .symbol = sym });
                },
                .return_const => {},
                .@"resume" => {},
    ...
    return struct {
        ...
        pub fn step(self: *Self, insn: bytecode.Insn, consts: []const object.Object) void {
            switch (insn) {
                ...
                .load_name => {
                    // TODO: resolve symbol via intern pool / globals — stub
                },
                .store_name => {
                    // TODO: resolve symbol via intern pool / globals — stub
                    _ = self.stack.pop();
