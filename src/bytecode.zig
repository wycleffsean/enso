...
test "bytecode: binary ops" {
    ...
    {
        // and == JUMP_IF_FALSE
        var ctx = try testSetup("True and False");
        defer testTeardown(&ctx);
    ...
    {
        // or == JUMP_IF_TRUE
        var ctx = try testSetup("True or False");
        defer testTeardown(&ctx);
    ...
    {
        // chain
        var ctx = try testSetup("True and False or True");
        defer testTeardown(&ctx);
...
test "bytecode: conditional expression" {
    ...
    {
        var ctx = try testSetup("1 if True else 2");
        defer testTeardown(&ctx);
